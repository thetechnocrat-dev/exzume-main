...
var FindInfluencersButton = React.createClass({
  ...
  corrSuccess: function (res) {
    ...
    console.log(res);
    var currentFeatureName = this.props.userFeature.name;
    ...
    var title = 'Top Correlations with ' + this.props.userFeature.name;

    FastFlux.cycle('BAR_OPTIONS_RECEIVED', { xAxisLabel: 'features', yAxisLabel: 'correlation' });
    FastFlux.cycle('BAR_TITLE_RECEIVED', title);
  ...
  render: function () {
    return (
      <div className="item" onClick={this.handleClick}>{this.props.userFeature.name}</div>
    );
  },
});
